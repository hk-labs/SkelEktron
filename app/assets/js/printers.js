...
ipc.on('printer-list', (event, printers) => {
  ...
  const printersListBox = document.getElementsByClassName('printer-list');
  if (printersListBox.length === 0) {
    return;
  }
  printersListBox[0].className += ' printer-list-active';

  let html = '<p>No printers found.</p>';
  ...
  if (printers.length > 0) {
    ...
    printers.forEach(function(device, i) {
      const isDefault = device.isDefault ? ' <small>(default)</small>' : '';
      const deviceName = device.options && device.options['printer-info']
        ? device.options['printer-info']
        : device.name;
      let buttons = `
<button class="printTXTTest" data-printer="${i}">Print TXT Test</button>
&nbsp;
<button class="printPDFTest" data-printer="${i}">Print PDF Test</button>
`;
